...
| H3 / h3-py 4.5.0 | Locked hierarchical spatial cells at resolutions 3, 5, and 7 | Apache-2.0 | [h3-py 4.5.0 licence](https://github.com/uber/h3-py/blob/v4.5.0/LICENSE). Preserve the licence and applicable upstream notices. Exact source/wheel hashes and runtime/core versions are recorded in `uv.lock`, `provenance/dependency_licenses.json`, and the ALA aggregation manifest. |
| rfc8785 0.1.4 | Locked RFC 8785 JSON Canonicalization Scheme implementation for Python semantic fingerprints | Apache-2.0 | [rfc8785.py source](https://github.com/trailofbits/rfc8785.py). Preserve the bundled Apache-2.0 licence and applicable notices. Exact source and universal-wheel hashes are recorded in `uv.lock` and `provenance/dependency_licenses.json`. |
